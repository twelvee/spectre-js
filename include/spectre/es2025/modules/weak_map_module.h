...
namespace spectre::es2025 {
    ...
    class WeakMapModule final : public Module {
    public:
        using Handle = std::uint64_t;

        struct Value {
            friend class WeakMapModule;
        ...
        StatusCode Set(Handle handle, ObjectModule::Handle key, const Value &value);

        StatusCode Get(Handle handle, ObjectModule::Handle key, Value &outValue) const;

        bool Has(Handle handle, ObjectModule::Handle key) const;

        StatusCode Delete(Handle handle, ObjectModule::Handle key, bool &outDeleted);
    ...
    private:
        ...
        MapRecord *FindMutable(Handle handle) noexcept;

        const MapRecord *Find(Handle handle) const noexcept;
        ...
        static std::uint64_t HashKey(ObjectModule::Handle key) noexcept;

        void Touch(MapRecord &record) noexcept;

        void TouchMetrics() noexcept;

        void EnsureCapacity(MapRecord &record);

        void Rehash(MapRecord &record, std::uint32_t newBucketCount);

        std::uint32_t Locate(const MapRecord &record, ObjectModule::Handle key, std::uint64_t hash,
                             std::uint32_t &bucket, bool &collision) const;

        std::uint32_t AllocateEntry(MapRecord &record);

        StatusCode Insert(MapRecord &record, ObjectModule::Handle key, const Value &value, std::uint64_t hash,
                          bool allowReplace);

        StatusCode Remove(MapRecord &record, ObjectModule::Handle key, std::uint64_t hash, bool &outDeleted);

        void PruneInvalid(MapRecord &record);
    };
}