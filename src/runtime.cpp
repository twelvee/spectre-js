...
namespace spectre {
    ...
    StatusCode SpectreRuntime::CreateContext(const ContextConfig &config, SpectreContext **outContext) {
        ...
        auto insertResult = m_Impl->m_Contexts.emplace(config.name,
                                                       SpectreContext(config.name, config.initialStackSize));
    ...
    EvaluationResult SpectreRuntime::LoadBytecode(const std::string &contextName, const BytecodeArtifact &artifact) {
        ...
        ScriptRecord record;
        record.source = {};
        record.bytecodeHash = std::to_string(
            std::hash<std::string>{}(std::string(artifact.data.begin(), artifact.data.end())));
